// Pure credit-policy math shared by the dashboard and the simulator.
// This mirrors contracts/sol/CreditPolicy.sol exactly (integer semantics),
// so the UI can show a verifiable, transparent breakdown with no wallet.

export const UNIT = 10n ** 6n; // USD values are 6-decimal on-chain
export const TIERS = ['Unscored', 'Starter', 'Established', 'Prime'];
...
export const clamp = (v) => Math.max(0, Math.min(1000, Math.floor(v)));
export const usd = (n) => '$' + (Number(n) / 1e6).toLocaleString('en-US', { maximumFractionDigits: 0 });
export const usd2 = (n) => '$' + (Number(n) / 1e6).toLocaleString('en-US', { maximumFractionDigits: 2 });
export const short = (a) => (a && a.length > 12 ? a.slice(0, 6) + '…' + a.slice(-4) : a || '—');

/**
 * @param {{collateralUsd:number,totalRepaidUsd:number,repaymentCount:number,onTimeCount:number,incomeUsd:number}} p
 * @returns {{score:number,tier:number,limit:number,components:{onTime:number,repaid:number,collateral:number,income:number}}}
 */
